/*
   ...
   1. Create a Prefix Sum array (pSum):
      - pSum[i] stores the count of odd numbers
        from index 0 to i.

   2. Build the Prefix Sum array:
      - Initialize pSum[0]:
          - If A[0] is odd, set pSum[0] = 1.
...
package Intermediate.Topic04_PrefixSum.Homework;
...
public class OddNumbersInRange {
    ...
    public static int[] solve(int[] A, int[][] B) {
        // An empty array has no odd numbers to count.
        if (A.length == 0) {
            return new int[0];
        }
        ...
        // Initialize the prefix count based on whether the first element is odd.
        if (A[0] % 2 != 0) {
            pSum[0] = 1;
        } else {
            pSum[0] = 0;
        ...
        // Return the count of odd numbers for each query.
        return result;
    }
}
