...
A `90` degree clockwise rotation can be performed in-place using two steps:

1. **Transpose the matrix.**
   - Swap `A[i][j]` with `A[j][i]`.
   - Traverse only the upper triangular part so that every pair is swapped exactly once.

2. **Reverse every row.**
   - After transposing, reverse each row from left to right.
...
The matrix is rotated **in-place**, so no additional matrix is created.
