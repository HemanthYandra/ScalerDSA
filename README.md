# ScalerDSA

DSA problems and solutions from the Scaler Academy Core Curriculum, organized by module and topic.

## Structure

There are 2 modules: `Intermediate` and `Advanced`.
Each module contains topic folders (e.g. `Topic1_Time_Complexity`).
Each topic folder contains two subfolders:
- `Assignment` — in-class assignment solutions
- `Homework` — homework problem solutions

## Modules

- **Intermediate** — Time Complexity, Arrays, Prefix Sum, Carry Forward, Sliding Window, Sorting, Bit Manipulation, 2D Matrices, Strings, and more.
- **Advanced** — One-Dimensional Arrays, Two-Dimensional Arrays, and more.

## Notes

Solutions target a Java judge environment (Scaler/InterviewBit platform), so some files favor manual array-backed structures, index-based loops, and explicit typing over newer Java syntax for judge compatibility.